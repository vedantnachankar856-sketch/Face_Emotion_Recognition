...
with st.sidebar:
    st.header("ℹ️ About")
    st.markdown("**Model:** CNN trained on FER2013")
...
if model is None:
    st.warning("⚠️ Model file not found. Place model_file.h5 in the repo root and redeploy.")
    ...
    if uploaded_file:
        ...
        if model is not None:
            ...
            if num_faces == 0:
                ...
                for r in results:
                    with st.expander(
                        f"Face #{r['face_id']}  —  {EMOTION_EMOJI[r['emotion']]} **{r['emotion']}** ({r['confidence']:.1%})",
                        expanded=True
                    ):
